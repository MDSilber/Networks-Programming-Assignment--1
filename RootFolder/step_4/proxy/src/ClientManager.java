...
public class ClientManager implements Runnable
{
	...
	ConcurrentHashMap<String,Integer> bannedUNIs;
	...
	public ClientManager(SemBoundedBuffer toServer, SemBoundedBuffer toClient, ServerSocket welcome)
	{
		...
		bannedUNIs = new ConcurrentHashMap<String,Integer>();
		clientCounter = 0;
	}
	...
	public void toClient(Client thisClient)
	{
		while(true)
		{
			...
			if(s != null)
			{
				...
				tokenizer.nextToken();
				String messageUNI = tokenizer.nextToken();

				System.out.println("Message UNI: " + messageUNI);
				//System.out.println("Message to send to client: " + s);
				if(bannedUNIs.containsKey(messageUNI))
				{
					System.out.println(messageUNI + " is a banned UNI! Discarding this message");
					continue;
				}
				else
				{
	...
	public void fromClient(Client thisClient)
	{
		while(true)
		{
			...
			if(s != null)
			{
				if(s.substring(0,2).equals("-a") || s.substring(0,2).equals("-r"))
				{
					System.out.println("Received UNIs! " + s);
					handleUNIs(s);
					s = null;
					continue;
				}
	...
	public void handleUNIs(String UNIs)
	{
		StringTokenizer tokenizer = new StringTokenizer(UNIs,":,");
		...
		if(key.equals("-a"))
		{
			while(tokenizer.hasMoreTokens())
			{
				...
				bannedUNIs.put(next, Integer.parseInt("1"));
			}
		}
		else
		{
			...
			{
				String next = tokenizer.nextToken();
				bannedUNIs.remove(next);
			}
		}

		Enumeration<String> keys = bannedUNIs.keys();
